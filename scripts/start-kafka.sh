...
# * BROKER_ID: configure the name of the broker
...
if [ ! -z "$KAFKA_LISTENERS" ]; then
  ...
  if grep -q "^listeners=" $KAFKA_HOME/config/server.properties; then
       sed -r -i "s~(^listeners)=(.*)~\1=$KAFKA_LISTENERS~g" $KAFKA_HOME/config/server.properties
  else
       echo "\nlisteners=$KAFKA_LISTENERS" >> $KAFKA_HOME/config/server.properties
  fi
fi
